...
pub async fn resolve_icon_url_from_image_name(image_tag: &str) -> Option<String> {
    ...
    let db_res = match db_res {
        Ok(res) => res,
        Err(e) => {
            println!("Failed to resolve icon for image {} (db error) | {}", image_tag, e);
            return None;
        },
    };
    ...
    let db_res = match db_res {
        Some(res) => res,
        None => {
            println!("Failed to resolve icon for image {} (no db response)", image_tag);
            return None;
        },
    };
    ...
    let caps = match caps {
        ...
        None => {
            println!("Failed to resolve icon for image {} (no regex match) for URL {}", image_tag, url.clone());
            return None;
        },
    };
    ...
    let icon_url = match icon_url {
        Some(res) => res.as_str(),
        None => {
            println!("Failed to resolve icon for image {} (no icon url)", image_tag);
            return None;
        },
    };
